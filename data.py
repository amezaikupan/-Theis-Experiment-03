...
class gauss_tl(object):
    ...
    # For experiment 03. test 02
    def task_shuffle(self):
        """
        Shuffle the data across tasks so that each task's data is randomized
        and not from the same distribution.
        """
        # Shuffle training data
        shuffled_indices = np.random.permutation(self.train["x_train"].shape[0])
        self.train["x_train"] = self.train["x_train"][shuffled_indices]
        self.train["y_train"] = self.train["y_train"][shuffled_indices]

        # Shuffle test data
        shuffled_indices_test = np.random.permutation(self.train["x_test"].shape[0])
        self.train["x_test"] = self.train["x_test"][shuffled_indices_test]
        self.train["y_test"] = self.train["y_test"][shuffled_indices_test]

        # Shuffle test set for the test dictionary
        shuffled_indices_test_train = np.random.permutation(self.test["x_train"].shape[0])
        self.test["x_train"] = self.test["x_train"][shuffled_indices_test_train]
        self.test["y_train"] = self.test["y_train"][shuffled_indices_test_train]

        shuffled_indices_test_test = np.random.permutation(self.test["x_test"].shape[0])
        self.test["x_test"] = self.test["x_test"][shuffled_indices_test_test]
        self.test["y_test"] = self.test["y_test"][shuffled_indices_test_test] 


    # For experiment 03. test 03
    def re_cluster(self, n_clusters=5):
        """
        Apply K-Means clustering to x_train, reorder x_train and y_train
        based on cluster assignments, and store the number of samples per cluster.
        """
        from sklearn.cluster import KMeans

        x_train = self.train["x_train"]
        y_train = self.train["y_train"]

        # Combine features and labels for clustering (optional)
        ...
        # Sort indices based on cluster_ids
        sorted_indices = np.argsort(cluster_ids)

        # Reorder x_train, y_train, and cluster_ids
        self.train["x_train"] = x_train[sorted_indices]
        self.train["y_train"] = y_train[sorted_indices]
        self.train["cluster_ids"] = cluster_ids[sorted_indices]

        # Count samples per cluster (after sorting)
        unique_clusters, counts = np.unique(cluster_ids, return_counts=True)
        self.train['n_samples_per_task'] = counts